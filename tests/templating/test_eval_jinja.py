...
def test_templating_variable_not_defined_throws_nameerror():
    with pytest.raises(NameError):
        _ = templating.evaluate('{{ var_1 }}', {
            'var_2': 1,
        })

def test_jinja_pipe():
    actual = templating.evaluate('{#jinja#}{{var1 | upper}}', {
        'var1': 'hello'
    })
    assert actual == "HELLO"
...
def test_python_string_interpolation():
    actual = templating.evaluate('hello {{var1}}{{"".join(var2)}}', {
        'var1': 'w',
        'var2': ['o','r','l','d'],
    })
...
def test_python_spaces():
    actual = templating.evaluate(' s {{  var_1 }}{{  (var_2  ) }}', {
        'var_1': 1,
        'var_2': 2,
    })
    assert actual == ' s 12'
